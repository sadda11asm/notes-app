...
function KeyService() {
  this.client = redis.createClient(config.get('key_service:port'), config.get('key_service:host'));
...
KeyService.prototype.set = function (user, deviceId) {
  ...
  var setExpiration = setKey.then(this.client.expireAsync(key, EXPIRATION_TIME));
